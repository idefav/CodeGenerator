...
namespace Idefav.DbObjects.OleDb
{
    public class DbObject:IDbObject
    {
        ...
        public T DbExcute<T>(Func<IDbCommand, T> proc, IDbTransaction transaction = null)
        {
            if (transaction != null)
            {
                using (OleDbCommand cmd = (OleDbCommand)transaction.Connection.CreateCommand())
                {
        ...
        public int ExceuteSql(string sql, IDbTransaction transaction = null, params KeyValuePair<string, object>[] parameters)
        {
            return DbExcute(cmd =>
            {
                OleDbCommand sqlcmd = cmd as OleDbCommand;
        ...
        public bool ExceuteTrans(Func<IDbTransaction, bool> proc)
        {
            return DbConnect(conn =>
            {
                ...
                OleDbTransaction transaction = (conn as OleDbConnection).BeginTransaction();
                try
                {
        ...
        {
            OleDbConnection connection = new OleDbConnection(DbConnectStr);
            connection.Open();
            ...
            params KeyValuePair<string, object>[] parameters)
        {
            ...
            return DbExcute(cmd =>
            {
                OleDbCommand orcmd = cmd as OleDbCommand;
                cmd.CommandType = CommandType.Text;
                ...
                OleDbDataAdapter adp = new OleDbDataAdapter(orcmd);
                DataSet ds = new DataSet();
                adp.Fill(ds);
                return ds.Tables[0];
            });
        ...
        public List<IDbDataParameter> MakeParams(KeyValuePair<string, object>[] parameters)
        {
            ...
            if (parameters != null)
            {
                ...
                {
                    OleDbParameter sqlParameter = new OleDbParameter(kv.Key, kv.Value);
